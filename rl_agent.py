...
class RlAgent:
    ...
    def __init__(self, input_dim, output_dim, save_dir=None, use_dueling=True):
        ...
        self.optimizer = optim.Adam(
            self.policy_net.parameters(), lr=2.5e-4, amsgrad=True
        )
    ...
    def act(self, state):
        ...
        # EXPLOIT: Best action from policy network
            ...
            state = (
                torch.tensor(np.array(state), device=self.device).unsqueeze(0).float()
                / 255.0
            )
            with torch.no_grad():
    ...
    def learn(self):
        ...
        # Check if we have enough memory to start learning
        if self.curr_step < self.burnin:
            return None, None

        # Learn only every few steps for stability
        if self.curr_step % self.learn_every != 0:
            return None, None

        # Anneal beta from 0.4 to 1.0 for bias correction
        beta_progress = min(1.0, self.curr_step / self.beta_frames)
        current_beta = self.beta_start + beta_progress * (1.0 - self.beta_start)
        self.memory._beta = current_beta
        ...
        # Calculate weighted loss with importance sampling
        ...
        if grad_norm > 10.0 and self.curr_step % 1000 == 0:
            print(f"Warning: grad_norm={grad_norm:.2f}")
        ...
        with torch.no_grad():
            ...
            new_priorities = (td_errors + 1e-6).clamp(max=1e2)
